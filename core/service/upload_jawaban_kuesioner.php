...
if(!isset($_FILES['jawaban'])){
	...
	$excel = ExcelReader::init($_FILES['jawaban']['tmp_name']);
	$row = $excel->rowcount($sheet_index = 0);

	$database->query('SELECT id, tipe_karakter FROM data_soal');
	$dataSoal = $database->resultSet();

	for($i=2; $i<$row; $i++){
		$idUser = $excel->val($i, 2);
		$jawaban = array();
		for($j=3; $j<43; $j++){
			array_push($jawaban, $excel->val($i, $j));
		}
		...
		foreach($jawaban as $key){
			$jawabanIndex = array_search($key, array_column($dataSoal, 'id'));
			switch($dataSoal[$jawabanIndex]['tipe_karakter']){
				case 'Sanguin': $value = 'A'; break;
				case 'Koleris': $value = 'B'; break;
				case 'Melankolis': $value = 'C'; break;
				case 'Plegmatis': $value = 'D'; break;
			}
